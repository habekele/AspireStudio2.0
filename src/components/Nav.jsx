...
export default function Nav() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 bg-bg/95 backdrop-blur-sm transition-all duration-300 ${scrolled ? 'border-b border-border shadow-xs' : ''}`}>
      <nav className="max-w-site mx-auto px-6 md:px-8 h-[68px] flex items-center justify-between">

        <a href="#top" className="flex flex-col">
          <span className="font-serif text-[1.25rem] font-medium tracking-[0.01em] text-text leading-none">Aspire Studios</span>
          <span className="font-sans text-[0.5rem] font-light tracking-[0.25em] uppercase text-muted mt-[3px]">Sioux Falls, SD</span>
        </a>
        ...
        <ul className="hidden md:flex items-center gap-9">
          ...
          <li>
            <a
              ...
              target="_blank"
              rel="noopener"
              className="font-sans text-[0.78rem] font-medium text-rose hover:text-rose-dark transition-colors duration-150"
            >
              Book Now
            </a>
          </li>
        ...
        >
          ...
          <span className={`block h-px bg-text transition-all duration-200 ${open ? 'w-5 opacity-0' : 'w-4'}`} />
          <span className={`block h-px bg-text transition-all duration-200 ${open ? 'w-5 -translate-y-[6px] -rotate-45' : 'w-5'}`} />
        </button>
      </nav>
      ...
      {open && (
        <div className="md:hidden bg-bg border-t border-border">
          <ul className="max-w-site mx-auto px-6 py-1 flex flex-col">
            ...
            <li className="py-3.5">
              <a href="https://aspirestudio.square.site/" target="_blank" rel="noopener" onClick={() => setOpen(false)} className="font-sans text-sm font-medium text-rose">
                Book Now
              </a>
            </li>
          </ul>
